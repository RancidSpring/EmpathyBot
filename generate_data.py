...
class Generate_data():
    ...
    def split_test(self):
        ...
        training_data = test.loc[test[" Usage"] == "Training"].drop(columns=[" Usage"]).rename(columns={" pixels": "pixels"})
        validation_data = test.loc[test[" Usage"] != "Training"].drop(columns=[" Usage"]).rename(columns={" pixels": "pixels"})
